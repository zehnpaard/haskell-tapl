...
showPTmAbsTest = TestList [
  "Test 1:" ~: (show $ PTmAbs "x" TpBool $ PTmVar "x") ~?= "λx:Bool.x",
  "Test 2:" ~: (show $ PTmAbs "x" TpBool $ PTmVar "y") ~?= "λx:Bool.y",
  "Test 3:" ~: (show $ PTmAbs "x" TpBool $ PTmAbs "y" TpBool $ PTmVar "z") ~?= "λx:Bool.λy:Bool.z"
  ]

showPTmAppTest :: Test
showPTmAppTest = TestList [
  "Test 1:" ~: (show $ PTmApp (PTmVar "x") (PTmVar "y")) ~?= "(x y)",
  "Test 2:" ~: (show $ PTmApp (PTmAbs "x" TpBool (PTmVar "x")) (PTmVar "y")) ~?= "(λx:Bool.x y)"
  ]
...
showPTmTrueFalseTest = TestList [
  ...
  "Test 3:" ~: (show $ PTmAbs "x" TpBool PTmTrue) ~?= "λx:Bool.true",
